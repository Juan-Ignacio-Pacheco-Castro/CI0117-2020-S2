...
GtkImage* matrix_images[NUM_LANES][NUM_OBSTACLES];
    ...
    if (running_track) {
        ...
        for (int i = 0; i < NUM_LANES; ++i) {
            ...
            for (int j = 0 ; j < NUM_OBSTACLES ; ++j){
                if (running_track->matrix_obstacles[i][j]){
                    gtk_image_set_from_file(GTK_IMAGE(matrix_images[i][j]), "sprite/deoxys.png");
                    //gtk_image_set_from_pixbuf(GTK_IMAGE(matrix_images[i][j]), gdk_pixbuf_scale_simple(gtk_image_get_pixbuf());
                } else {
                    gtk_image_clear(GTK_IMAGE(matrix_images[i][j]));
                }
            }
         ...
         gpointer user_data)
{
    ...
    /* place the button in the grid cell (3, 9), width=3 heigth=1 */
    ...
    for (int i = 0; i < NUM_LANES; ++i){
        ...
        for (int j = 0; j < NUM_OBSTACLES; ++j){
            int pos_x = j + 2;
            matrix_images[i][j] = gtk_image_new();
            gtk_grid_attach(GTK_GRID(grid), matrix_images[i][j], pos_x, pos_y, 1, 1);
        }
    }
    ...
    for (int i = 0; i < NUM_LANES; ++i) {
        ...
        gtk_grid_attach(GTK_GRID(grid), labels_finish_line[i], 10, i+1, 1, 1);
    }
