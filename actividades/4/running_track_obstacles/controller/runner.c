...
runner_t* runner_create(int id, int preparation_time, int running_time) {
    ...
    runner->time_per_obstacle = running_time/NUM_OBSTACLES;
...
void* run(void* args) {
    ...
    *data->lane_start = 1;
    ...
    while(obstacle_position < NUM_OBSTACLES){
        fixed_sleep((useconds_t)data->runner->time_per_obstacle);
        pthread_mutex_lock(data->mutex_position);
        data->obstacle_matrix[data->runner->id][obstacle_position] = 1;
        obstacle_position-1 >=0 ? data->obstacle_matrix[data->runner->id][obstacle_position-1]-- : 0;
