...
array_t* array_create(size_t capacity)
{
	...
	pthread_mutex_init(&array->mutex_array, NULL);
...
void array_destroy(array_t* array)
{
	...
	pthread_mutex_destroy(&array->mutex_array);
	free(array->elements);
	free(array);
}
...
{
	assert(array);
	pthread_mutex_lock(&array->mutex_array);
	size_t count_temp = array->count;
	pthread_mutex_unlock(&array->mutex_array);
	return count_temp;
}
