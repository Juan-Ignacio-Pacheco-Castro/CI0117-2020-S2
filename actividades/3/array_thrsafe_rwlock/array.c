...
array_t* array_create(size_t capacity)
{
	...
	pthread_rwlock_init(&array->rwlock_array, NULL);
...
void array_destroy(array_t* array)
{
	...
	pthread_rwlock_destroy(&array->rwlock_array);
	free(array->elements);
	free(array);
}
...
{
	assert(array);
	pthread_rwlock_rdlock(&array->rwlock_array);
	size_t count_temp = array->count;
	pthread_rwlock_unlock(&array->rwlock_array);
	return count_temp;
}
...
void* array_get_element(array_t* array, size_t index)
{
	...
	pthread_rwlock_rdlock(&array->rwlock_array);
	void* element_temp = array->elements[index];
	pthread_rwlock_unlock(&array->rwlock_array);
	return element_temp;
}
...
int array_append(array_t* array, void* element)
{
	...
	pthread_rwlock_wrlock(&array->rwlock_array);
