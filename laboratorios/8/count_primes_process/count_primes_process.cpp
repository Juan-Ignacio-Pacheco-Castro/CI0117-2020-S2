...
int main(int argc, char *argv[]) {
    ...
    if(myRank != 0)
        MPI_Recv(&limit, 1, MPI_INT, 0, 123, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    else{
        if (argc >= 2){
    ...
    if(myRank == 0){
        ...
        begin = end + 1;
        int possibleEnd = begin + delta;
        end = (possibleEnd > limit)?limit:possibleEnd;
    ...
    if(myRank)
        MPI_Send(&counter, 1, MPI_INT, 0, 123, MPI_COMM_WORLD);
    else{
        totalPrimes += counter;
        for(int rank = 1; rank < numProcesses; ++rank){
            MPI_Recv(&counter, 1, MPI_INT, rank, 123, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            totalPrimes += counter;
        }
        endwtime = MPI_Wtime();
        std::cout << totalPrimes << " primes found in range [2, " << limit << "] in " << endwtime - startwtime << "s with " << numProcesses << " processes\n";
    }
